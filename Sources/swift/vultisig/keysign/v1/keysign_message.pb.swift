...
public struct VSKeysignMessage {
  ...
  /// Returns true if `customPayloadID` has been explicitly set.
  public var hasCustomPayloadID: Bool {return self._customPayloadID != nil}
...
public struct VSKeysignPayload {
  ...
  public var signSolana: VSSignSolana {
    get {
      if case .signSolana(let v)? = _storage._signData {return v}
      return VSSignSolana()
    }
  ...
  public var signBitcoin: VSSignBitcoin {
    get {
      if case .signBitcoin(let v)? = _storage._signData {return v}
      return VSSignBitcoin()
    }
    ...
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      while let fieldNumber = try decoder.nextFieldNumber() {
        ...
        case 41: try {
          ...
          if let current = _storage._signData {
            hadOneofValue = true
            if case .signSolana(let m) = current {v = m}
          }
          try decoder.decodeSingularMessageField(value: &v)
          if let v = v {
            if hadOneofValue {try decoder.handleConflictingOneOf()}
        ...
        case 43: try {
          ...
          if let current = _storage._signData {
            hadOneofValue = true
            if case .signBitcoin(let m) = current {v = m}
          }
          try decoder.decodeSingularMessageField(value: &v)
          if let v = v {
            if hadOneofValue {try decoder.handleConflictingOneOf()}
